...
<main class="flex flex-col justify-center p-6 pb-12">
    <div class="mx-auto max-w-md">
        @guest()
            <h2 class="mt-2 text-2xl font-bold text-gray-700 sm:mt-6 sm:text-3xl">Сброс пароля</h2>
        @endguest
        @auth()
            <h2 class="mt-2 text-2xl font-bold text-gray-700 sm:mt-6 sm:text-3xl">Редактирование пароля</h2>
        @endauth
    </div>
    <div class="mx-auto mt-6 w-full max-w-md rounded-xl bg-white/80 p-6 shadow-xl backdrop-blur-xl sm:mt-10 sm:p-10">
        {{-- Форма для сброса пароля --}}
        <form action="{{ route('password.update') }}" novalidate method="post" autocomplete="off">
            @csrf
            {{-- скрытый токен, необходимый для сброса пароля --}}
            ...
            <div class="flex justify-center">
                @guest()
                    <button type="submit" class="flex w-full items-center justify-center rounded-md bg-blue-400 py-2 px-4 font-semibold text-white shadow-lg transition duration-150 ease-in-out hover:bg-blue-500 hover:shadow-xl focus:shadow-xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2">Сбросить пароль</button>
                @endguest
                @auth()
                    <button type="submit" class="flex w-full items-center justify-center rounded-md bg-blue-400 py-2 px-4 font-semibold text-white shadow-lg transition duration-150 ease-in-out hover:bg-blue-500 hover:shadow-xl focus:shadow-xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2">Изменить пароль</button>
                @endauth
            </div>
