...
<header class="flex items-center justify-between p-6">
    ...
    <div>
        ...
        <a href="{{ route('register') }}" class="rounded-md bg-blue-400 py-2 px-4 font-semibold text-white shadow-lg transition duration-150 ease-in-out hover:bg-blue-600 hover:shadow-xl focus:shadow-xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2">Регистрация</a>
    </div>
</header>
<main class="flex flex-col justify-center p-6 pb-12">
    <div class="mx-auto max-w-md">
        <h2 class="mt-2 text-2xl font-bold text-gray-700 sm:mt-6 sm:text-3xl">Вход в аккаунт</h2>
    </div>
    <div class="mx-auto mt-6 w-full max-w-md rounded-xl bg-white/80 p-6 shadow-xl backdrop-blur-xl sm:mt-10 sm:p-10">
        {{-- Статус для сброса пароля --}}
        @if(session('status'))
            <div class="flex gap-3 rounded-md border border-blue-500 bg-blue-50 p-4 mb-6">
        ...
        <form action="{{ route('login') }}" method="post" autocomplete="off">
            ...
            {{-- Почта --}}
            <div class="mb-6">
                <label for="email" class="block text-xl font-medium text-gray-600">Почта</label>
                <div class="mt-1 rounded-md shadow-sm">
                    <input type="email" id="email" name="email" value="{{ old('email') }}" autofocus required class="w-full rounded-md pl-3 text-sm {{ $errors->has('email') ? ' border-red-300 text-red-900 placeholder-red-300 focus:border-red-500 focus:ring-red-500' : 'border-gray-300 focus:border-blue-400 focus:ring-blue-500 placeholder:text-gray-400' }}" placeholder="Ivanov@example.com" />
                </div>
                @error('email')
            ...
            {{-- Пароль --}}
            <div class="mr-1 mb-6">
                <label for="password" class="block text-xl font-medium text-gray-600">Пароль</label>
                <div class="mt-1 rounded-md shadow-sm">
            ...
            {{-- "Запомнить меня и ссылка для восстановления аккаунта" --}}
            <div class="flex items-center justify-between mb-6">
                <div class="flex items-center gap-2">
                    <input type="checkbox" id="remember" name="remember" class="h-4 w-4 rounded border-gray-300 text-blue-400" />
                    <label for="remember" class="text-sm text-gray-900">Запомнить меня</label>
                </div>
                <a href="{{ route('password.request') }}" class="text-sm font-medium text-blue-400 hover:text-blue-500">Забыли пароль?</a>
            </div>

            {{-- Вход --}}
            <div class="flex justify-center">
                <button type="submit" class="flex w-1/2 items-center justify-center rounded-md bg-blue-400 py-2 px-4 font-semibold text-white shadow-lg transition duration-150 ease-in-out hover:bg-blue-500 hover:shadow-xl focus:shadow-xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2">Войти</button>
            </div>

        </form>

        {{-- Ссылка на вкладку регистрации --}}
        <div class="mt-3 flex items-center justify-center">
            <a href="{{ route('register') }}" class="text-base font-medium text-blue-400 hover:text-blue-500">У вас нет аккаунта?</a>
        </div>
